...
> **Updated**: 2026-01-04 (SPECIFICATION.md v3.2対応)
...
## 3. tmuxキーバインド（v3.2で簡略化）

| 仕様書キー | 実装 | ステータス | 備考 |
|------------|------|------------|------|
| `prefix + t` | ✅ | 一致 | Navigator直接起動（v3.2で2段階から変更） |

**Note**: v3.2で `prefix + t, c/n/l/r/?` の2段階キーバインドは廃止。すべての操作はNavigator内で実行。
...
## 5. セッション状態（v3.2で2状態に簡略化）

| 状態 | アイコン | コード定数 | ステータス |
|------|----------|------------|------------|
| Active | `▶` | STATE_ACTIVE | ✅ 一致 |
| Dormant | `○` | STATE_DORMANT | ✅ 一致 |

**Note**: v3.2で`exited`状態は廃止。tmuxセッションが存在すれば`active`として扱う。Claude実行状態はセッション内で判断。
...
| 日付 | 内容 |
|------|------|
| 2026-01-04 | v3.2対応：2段階キーバインド廃止、セッション状態2状態化 |
