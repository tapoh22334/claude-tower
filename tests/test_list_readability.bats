...
@test "_strip_command_markup: unwraps a command expansion to its arguments" {
    run _strip_command_markup '<command-message>brainstorming</command-message> <command-name>/brainstorming</command-name> <command-args>make the transition nicer</command-args>'
    [ "$output" = "make the transition nicer" ]
}

@test "_strip_command_markup: drops tags when there are no args to keep" {
    run _strip_command_markup '<command-message>init</command-message><command-name>/init</command-name>'
    [ "$output" = "" ]
}

@test "_strip_command_markup: drops the local-command caveat but keeps the prompt" {
    run _strip_command_markup '<local-command-caveat>Caveat: The messages below were generated by the user while running local commands.</local-command-caveat>fix the header'
    [ "$output" = "fix the header" ]
}

@test "_first_meaningful_sentence: never emits command expansion markup" {
    run _first_meaningful_sentence '<command-message>x</command-message> <command-args>ボードサイズを直したい</command-args>'
    [ "$status" -eq 0 ]
    [ "$output" = "ボードサイズを直したい" ]
    [[ "$output" != *"<"* ]]
}

@test "get_session_title: falls back to a slash command's argument (regression: fork rows showed <command-message>)" {
    # A forked session whose only prompts are slash commands: previously
    # every history line was rejected and the transcript's raw command
    # expansion leaked into the row.
    CLAUDE_HISTORY_FILE="${BATS_TEST_TMPDIR}/history.jsonl"
    local uuid="33333333-3333-4333-8333-333333333333"
    echo '{"display":"/brainstorming ボードサイズが変わるときのUXを直したい","sessionId":"'"$uuid"'"}' > "$CLAUDE_HISTORY_FILE"
    run get_session_title "$uuid"
    [ "$status" -eq 0 ]
    [ "$output" = "ボードサイズが変わるときのUXを直したい" ]
    [[ "$output" != *"command-"* ]]
}

@test "get_session_title: prefers a real prompt over a slash command's argument" {
    CLAUDE_HISTORY_FILE="${BATS_TEST_TMPDIR}/history.jsonl"
    local uuid="44444444-4444-4444-8444-444444444444"
    {
        echo '{"display":"/fork do the thing","sessionId":"'"$uuid"'"}'
        echo '{"display":"make the build reproducible","sessionId":"'"$uuid"'"}'
    } > "$CLAUDE_HISTORY_FILE"
    run get_session_title "$uuid"
    [ "$status" -eq 0 ]
